...
std::unique_ptr<Camera> Dielectrics(Scene& scene)
{
    ...
    const Material* outers[count];
    outers[0] = scene.CreateMaterial<DielectricMaterial>(1.5f, ConstantFloatTexture::Create(0.02f));
    outers[1] = scene.CreateMaterial<DielectricMaterial>(1.5f, ConstantFloatTexture::Create(0.0f));
    outers[2] = scene.CreateMaterial<DielectricMaterial>(1.5f, ConstantFloatTexture::Create(0.05f));

    const Material* inners[count];
    inners[0] = scene.CreateMaterial<UnrealMaterial>(
        ConstantColorTexture::Create(0.66), ConstantFloatTexture::Create(0), ConstantFloatTexture::Create(0)
    );
    inners[1] = inners[0];
    inners[2] = inners[0];
...
std::unique_ptr<Camera> Mixtures(Scene& scene)
{
    ...
    const Material* outers[count];
    auto a = scene.CreateMaterial<DielectricMaterial>(1.5f, ConstantFloatTexture::Create(0.0f));
    auto b =
        scene.CreateMaterial<ConductorMaterial>(ConstantColorTexture::Create(0.7, 0.3, 0.2), ConstantFloatTexture::Create(0.1f));
    auto checker = FloatCheckerTexture::Create(ConstantFloatTexture::Create(0), ConstantFloatTexture::Create(1), Point2(20));
    ...
    auto d = scene.CreateMaterial<ConductorMaterial>(
        ConstantColorTexture::Create(0.7f), ConstantFloatTexture::Create(0.4f), ConstantFloatTexture::Create(0.05f)
    );
