...
std::unique_ptr<Camera> CornellBox(Scene& scene)
{
    ...
    // return std::make_unique<OrthographicCamera>(Transform::LookAt(position, target, y_axis), Point2(1.02, 1.02), width);
    // return std::make_unique<SphericalCamera>(position, Point2i(width, width));
