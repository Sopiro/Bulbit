...
std::unique_ptr<Camera> SSSTest2(Scene& scene)
{
    ...
    // Model
    {
        ...
        auto mat = scene.CreateMaterial<SubsurfaceMaterial>(r, ssc * 0.001, 1.5, 0.0);
        ...
        mat = scene.CreateMaterial<SubsurfaceMaterial>(r, ssc * 0.005, 1.5, 0.0);
        SetLoaderFallbackMaterial(mat);
        LoadModel(scene, "res/stanford/lucy.obj", tf);

        tf = Transform{ Vec3(0, 0, 0), Quat::FromEuler(0, 0, 0), Vec3(1) };
        mat = scene.CreateMaterial<SubsurfaceMaterial>(r, ssc * 0.008, 1.5, 0.0);
        SetLoaderFallbackMaterial(mat);
        LoadModel(scene, "res/stanford/lucy.obj", tf);

        tf = Transform{ Vec3(1 * d, 0, 0), Quat::FromEuler(0, 0, 0), Vec3(1) };
        mat = scene.CreateMaterial<SubsurfaceMaterial>(r, ssc * 0.01, 1.5, 0.0);
        SetLoaderFallbackMaterial(mat);
        LoadModel(scene, "res/stanford/lucy.obj", tf);

        tf = Transform{ Vec3(2 * d, 0, 0), Quat::FromEuler(0, 0, 0), Vec3(1) };
        mat = scene.CreateMaterial<SubsurfaceMaterial>(r, ssc * 0.02, 1.5, 0.0);
        SetLoaderFallbackMaterial(mat);
        LoadModel(scene, "res/stanford/lucy.obj", tf);
    }
