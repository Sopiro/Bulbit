...
ImageTexture<Float>* CreateFloatImageTexture(
    Scene& scene, std::string filename, int32 channel, bool non_color = false, std::function<Float(Float)> transform = {}
);
ImageTexture<Spectrum>* CreateSpectrumImageTexture(
    Scene& scene, std::string filename, bool non_color = false, std::function<Spectrum(Spectrum)> transform = {}
);
