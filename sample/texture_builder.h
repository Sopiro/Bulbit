...
ImageTexture<Float>* CreateFloatImageTexture(
    Scene& scene, std::string filename, int32 channel, bool non_color = false, Float multiplier = Float(1)
);
ImageTexture<Spectrum>* CreateSpectrumImageTexture(
    Scene& scene, std::string filename, bool non_color = false, Spectrum multiplier = Spectrum(1)
);
