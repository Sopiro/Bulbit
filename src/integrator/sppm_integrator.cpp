...
SpectrumSample SPPMIntegrator::SampleDirectLight(
    const Vec3& wo,
        ...
        for (int32 iteration = 0; iteration < n_iterations; ++iteration)
        {
            ...
            ParallelFor2D(
                ...
                [&](AABB2i tile) {
                    ...
                    for (Point2i pixel : tile)
                    {
                        ...
                        WavelengthSample path_lambda = iteration_lambda;
                        ...
                        while (true)
                        {
                            ...
                            if (!Intersect(&isect, ray, Ray::epsilon, infinity))
                            {
                                ...
                                if (bounce == 0 || specular_bounce)
                                {
                                    ...
                                    {
                                        L += beta * light->Le(ray, path_lambda);
                                    }
                                }
                                else
                                {
                                    for (Light* light : infinite_lights)
                                    {
                                        ...
                                        L += beta * mis_weight * light->Le(ray, path_lambda);
                                    }
                                }
                                ...
                                (IsNonSpecular(flags) && !sample_direct_light))
                            {
                                ...
                                vp.wo = wo;
                                vp.wavelength_collapsed = path_lambda.IsCollapsed();
                                vp.bsdf = bsdf;
                                vp.beta = beta;
            ...
            ParallelFor(0, photons_per_iteration, [&](int32 begin, int32 end) {
                ...
                for (int32 i = begin; i < end; ++i)
                {
                    ...
                    if (!light->Sample_Le(&light_sample, sampler->Next2D(), sampler->Next2D(), iteration_lambda))
                    {
                        continue;
                    }
        ...
        ParallelFor2D(
            ...
            [&](AABB2i tile) {
                for (Point2i pixel : tile)
                {
                    ...
                    Vec3 L = (vp.Ld / n_iterations) + vp.tau / (total_photons * pi * Sqr(vp.radius));
