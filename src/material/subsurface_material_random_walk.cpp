...
SubsurfaceMaterialRandomWalk::SubsurfaceMaterialRandomWalk(
    ...
    : SubsurfaceMaterialRandomWalk(reflectance, mfp, eta, ConstantFloatTexture::Create(roughness), normalmap)
{
}
...
SubsurfaceMaterialRandomWalk::SubsurfaceMaterialRandomWalk(
    ...
    : SubsurfaceMaterialRandomWalk(reflectance, mfp, eta, roughness, roughness, normalmap)
{
}
...
{
    Spectrum Sp = reflectance->Evaluate(isect.uv);
    Spectrum sigma_t = 1 / l;

    *bssrdf = alloc.new_object<RandomWalkBSSRDF>(Sp, Spectrum(0), sigma_t, isect, wo, eta);
    return true;
}
