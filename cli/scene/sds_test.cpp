...
void SDSTest(RendererInfo* ri)
{
    ...
    // Model
    {
        auto glass = CreateDielectricMaterial(scene, Spectrum::CauchyIOR(1.5f, 0.1f));
...
void SDSTest2(RendererInfo* ri)
{
    ...
    // Wall
    {
        auto mirror = CreateMirrorMaterial(scene, Spectrum(0.9));
        auto tf = Transform{ Vec3(-1, .5f, -.5), Quat::FromEuler({ 0, pi / 8, 0 }), Vec3(1.5, 1, 1) };
        CreateRectXY(scene, tf, mirror);
    }

    {
        ...
        auto tf = Transform{ 0, 2.0f, 0, Quat(pi, x_axis), Vec3(1.5f) };
        // CreateRectXZ(scene, tf, white, {}, AreaLightInfo{ .type = AreaLightType::spot, .emission = 5.0f });
        CreateSpotLight(scene, Point3(2, 4, -1.5), Point3(2, 4, -1.5), 50, 45, 30);
    }
    ...
    Point3 position{ -1.5, 1.0, 3 };
    Point3 target{ -.5, 0.3, 0.0 };
...
static int32 index2 = Sample::Register("sds2", SDSTest2);